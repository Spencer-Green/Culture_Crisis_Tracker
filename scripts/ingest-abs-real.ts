...
import { parseAbsRealCliArguments } from "../src/data-sources/macro/abs-real-cli";
import { validateAbsQuarterRange } from "../src/data-sources/macro/abs-period";
...
async function main() {
  const options = parseAbsRealCliArguments(process.argv.slice(2));
  const range = validateAbsQuarterRange(options.startPeriod, options.endPeriod);

  console.log("Source: ABS Quarterly Household Spending Indicator (HSI_Q)");
